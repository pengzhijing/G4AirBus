    ...
    public com.bigkoo.alertview.OnItemClickListener updateclick=new com.bigkoo.alertview.OnItemClickListener(){
        public void onItemClick(Object o,int position) {
            if(position==-1){
                ...
                progress.show();
                if (data.size() != 0) {
                    MainActivity.mgr.deletesong("song", FounctionActivity.roomidfc,thismusic.music_id);
                }
                updatestep = 1;
                getdatahandler.postDelayed(updatarun, 10);
            }
    ...
    Runnable getdatarun=new Runnable() {
        ...
        public void run() {
            ...
            for(int i=0;i<alldata.size();i++){
                if(alldata.get(i).room_id==FounctionActivity.roomidfc&&alldata.get(i).music_id==thismusic.music_id){
            ...
            for(int i=0;i<musicx.size();i++){
                if(musicx.get(i).room_id==FounctionActivity.roomidfc&&musicx.get(i).music_id==thismusic.music_id){
