 ...
 */
public class OtherType4 extends LinearLayout implements View.OnClickListener,View.OnLongClickListener{
    ...
    public OtherType4(Context context) {
        super(context);
        // 将自定义组合控件的布局渲染成View
        initview(context);
    }
    public OtherType4(Context context, AttributeSet attrs) {
        super(context, attrs);
        initview(context);
    }
                ...
                switch(item.getItemId()){
                    case R.id.action_open:
                        settingalter = new AlertView("Settings", null, "CANCEL",  new String[]{"SAVE"}, null, rootcontext, AlertView.Style.Alert,
                                settingclick);
                        View selfview= inflater.inflate(R.layout.setting_otherinfotype4, null);
...
// 如何处理灯的开关，需要参数，1.灯位置 2.灯类型，是什么样的灯，如果是开灯，就关灯，如果是关灯就开灯，
    public void HandleLightOnoff(int areaNo,int sceneNo) {
        ...
        try {
            ...
            // 执行开关灯
            if (oc.SceneControl(byteSubnetID, byteDeviceID,
                    areaNo,sceneNo,MainActivity.mydupsocket) == true) {
                bytSuccess = 1;
