...
public class MusicAdapter extends BaseAdapter{
    ...
    private List<Savemusic> buttonarray=new ArrayList<Savemusic>();
    private int size;
    private LayoutInflater inflater=null;
    private HashMap<Integer, View> mView ;
    public MusicAdapter(final Context context, List<Savemusic> list){
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(view==null)
        {
            ...
            String musicRemark=buttonarray.get(position).music_remark.trim();
            if (musicRemark.equals("")){
                txt.setText(buttonarray.get(position).subnetID+"_"+buttonarray.get(position).deviceID);
            }else{
                txt.setText(buttonarray.get(position).music_remark);
            }
            ceb.setImageDrawable(context.getResources().getDrawable(getResourdIdByResourdName(context,"music")));
